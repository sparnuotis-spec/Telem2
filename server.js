...
app.post('/api/drone-status',(req,res)=>{const body=req.body||{};const allowed=['disconnected','connecting','verified','mass_storage','error'];const status=String(body.state||'').toLowerCase();if(!allowed.includes(status))return res.status(400).json({error:'Invalid drone status.'});droneStatus={state:status,port:String(body.port||'').slice(0,20),computer:String(body.computer||'').slice(0,80),operator:String(body.operator||'').slice(0,80),message:String(body.message||status).slice(0,160),updated_at:Date.now()};droneStatusHistory.unshift({...droneStatus});if(droneStatusHistory.length>20)droneStatusHistory.pop();broadcast();res.json({ok:true,drone_status:droneStatus});});
