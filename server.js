...
app.post('/api/sd-transfer/:pilot/confirm', (req, res) => {
  const pilotId=req.params.pilot; const sessionId=Number(req.body?.session_id||0); const pilot=db.prepare('SELECT tag FROM pilots WHERE pilot_id=?').get(pilotId); const batchSize=pilot?.tag==='sd card'?4:2; const flights=db.prepare("SELECT * FROM flights WHERE session_id=? AND pilot_id=? AND mode='DYN' AND status='completed' AND result='needs_sd_transfer' AND sd_transfer_ack=0 ORDER BY id").all(sessionId,pilotId); if(flights.length!==batchSize)return res.status(400).json({error:`Exactly ${batchSize} completed flights are required.`});
...
app.post('/api/flights/:id/files', upload.array('files', 10), async (req, res) => {
  ...
  const pilot=db.prepare('SELECT tag FROM pilots WHERE pilot_id=?').get(flight.pilot_id); const completedCount=db.prepare("SELECT COUNT(*) AS n FROM flights WHERE session_id=? AND pilot_id=? AND mode='DYN' AND status='completed' AND result IN ('success','needs_sd_transfer')").get(flight.session_id,flight.pilot_id).n; const batchSize=pilot?.tag==='sd card'?4:2; if (pilot && completedCount < batchSize) return res.status(400).json({ error: `${pilot.tag==='sd card'?'SD-card':'Regular'} files unlock after this pilot completes ${batchSize} flights.` });
