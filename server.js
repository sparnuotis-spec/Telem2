...
app.post('/api/drone-status',(req,res)=>{const body=req.body||{};const allowed=['disconnected','connecting','verified','mass_storage','error'];const status=String(body.state||'').toLowerCase();if(!allowed.includes(status))return res.status(400).json({error:'Invalid drone status.'});const explicitOperator=String(body.operator||'').trim();const latestOperator=[...connectedUsers.values()].sort((a,b)=>b.updated_at-a.updated_at)[0]?.name||'';droneStatus={state:status,port:String(body.port||'').slice(0,20),computer:String(body.computer||'').slice(0,80),operator:(explicitOperator||latestOperator).slice(0,80),message:String(body.message||status).slice(0,160),updated_at:Date.now()};droneDevices.set(`${droneStatus.computer||'unknown'}:${droneStatus.port||'unknown'}`,droneStatus);broadcast();res.json({ok:true});});
...
app.get('/api/state', (req, res) => {
  ...
  res.json({ session, sessions, participants, pilots, uavs, rounds, scenarios, flights: flights.map(f => ({ ...f, display_name: flightName(f) })), files, events, transfer_presence:Object.fromEntries(transferPresence), connected_users:[...connectedUsers.values()], drone_status:droneStatus, drone_devices:[...droneDevices.values()] });
});
...
app.post('/api/sd-transfer/:pilot/confirm', (req, res) => {
  const pilotId=req.params.pilot; const sessionId=Number(req.body?.session_id||0); const pilot=db.prepare('SELECT tag FROM pilots WHERE pilot_id=?').get(pilotId); const batchSize=pilot?.tag==='sd card'?4:2; const flights=db.prepare("SELECT * FROM flights WHERE session_id=? AND pilot_id=? AND status='completed' AND result='needs_sd_transfer' AND sd_transfer_ack=0 ORDER BY id").all(sessionId,pilotId); if(flights.length!==batchSize)return res.status(400).json({error:`Exactly ${batchSize} completed flights are required.`});
