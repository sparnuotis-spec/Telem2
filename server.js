...
app.post('/api/drone-status',(req,res)=>{const body=req.body||{};const allowed=['disconnected','connecting','verified','mass_storage','error'];const status=String(body.state||'').toLowerCase();if(!allowed.includes(status))return res.status(400).json({error:'Invalid drone status.'});const explicitOperator=String(body.operator||'').trim();const latestOperator=[...connectedUsers.values()].sort((a,b)=>b.updated_at-a.updated_at)[0]?.name||'';droneStatus={state:status,port:String(body.port||'').slice(0,20),computer:String(body.computer||'').slice(0,80),operator:(explicitOperator||latestOperator).slice(0,80),message:String(body.message||status).slice(0,160),updated_at:Date.now()};broadcast();res.json({ok:true});});
...
app.get('/api/state', (req, res) => {
  ...
  res.json({ session, sessions, participants, pilots, uavs, rounds, scenarios, flights: flights.map(f => ({ ...f, display_name: flightName(f) })), files, events, transfer_presence:Object.fromEntries(transferPresence), connected_users:[...connectedUsers.values()], drone_status:droneStatus });
});
